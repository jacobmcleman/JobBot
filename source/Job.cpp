...
Job::Job(JobFunction function, Job *parent)
    : jobFunc_(function.function), callbackFunc_(nullptr), unfinishedJobs_(1),
      ...
      flags_(function.flags &
             ~(JOB_FLAG_MASK_STATUS_IN_PROGRESS |
               JOB_FLAG_MASK_STATUS_CANCELLED)) {
...
unsigned char Job::GetFlags() const { return flags_; }

JobFunction::JobFunction(JobFunctionPointer func, JobType type)
    : function(func), flags(0) {
  unsigned char flag = 0;
  if (type == JobType::Tiny)
    flag |= JOB_FLAG_MASK_TINY;
  if (type == JobType::Huge)
    flag |= JOB_FLAG_MASK_HUGE;
  if (type == JobType::IO)
    flag |= JOB_FLAG_MASK_IO;
  if (type == JobType::Graphics)
    flag |= JOB_FLAG_MASK_GRAPHICS;
