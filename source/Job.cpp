...
namespace JobBot
{
  ...
  std::atomic_size_t Job::sJobsCompleted_ = 0;
    ...
    Job::Job(JobFunction function, Job* parent) :
        jobFunc_(function.function),
        callbackFunc_(nullptr),
        unfinishedJobs_(1),
        parent_(parent),
        ghostJobCount_(0),
        flags_(function.flags & ~JOB_FLAG_MASK_IN_PROGRESS)
    {
        //If there is a parent, it now has one more job that must finish before parent is done
        if (parent_ != nullptr)
        {
  ...
  Job* Job::CreateChild(JobFunction& function, Job* parent)
  {
    ...
    do
    {
      ...
      //Use & as a bitmask instead of modulo since the array is a power of 2 in size
  ...
  ///There used to be an empty space here, thus causing Julian to be moderately miffed. I put a comment here, and now tis fine.

    void Job::Run()
    {
        //If there is a job function
        if (jobFunc_ != nullptr && !(flags_ & JOB_FLAG_MASK_IN_PROGRESS))
        {
    ...
    void Job::SetCallback(JobFunction func)
    {
        callbackFunc_ = func.function;
    }
    ...
    size_t Job::GetUnfinishedJobCount()
    {
        return sJobsAdded_ - sJobsCompleted_;
    }
#endif
    ...
    bool Job::GetCompletable() const
    {
        return ghostJobCount_;
    }

    unsigned char Job::GetFlags() const
    {
        return flags_;
    }
