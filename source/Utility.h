...
namespace JobBot
{
	...
	public:
		/*
			Specialized type of job for use with parallel for that takes some extra data

			dataChunk - array of the data that this job should execute over
			chunkSize - number of elements in that array

			Note that the nature of this function means there are likely elements before and after
			the assigned data chunk, and accessing them will likely not cause an error, but it is
			NOT THREAD SAFE
		*/
		template<typename T>
		using ParallelForJobFunction = void(*)(Job* job, T* dataChunk, size_t chunkSize);

		/*
			Creates a parallel for job

			Parallel-For executes the given ParallelForJobFunction on each element in the data array
			A job is created to handle every *chunkSize* elements
		*/
		template<typename T>
		static Job* ParallelForJob(Manager* manager, ParallelForJobFunction<T> function, T* data, size_t size, size_t chunkSize);
	...
	private:
		...
		*/
		template<typename T>
		struct ParallelForSplitterData
		{
		...
		*/
		template<typename T>
		static void ParallelForSplitterFunction(Job* job);
		...
		*/
		template<typename T>
		static void ParallelForJob(Job* job);
	};


	template<typename T>
	inline Job * Utilities::ParallelForJob(Manager* manager, ParallelForJobFunction<T> function, T * data, size_t size, size_t chunkSize)
	{
	...
	template<typename T>
	inline void Utilities::ParallelForJob(Job * job)
	{
